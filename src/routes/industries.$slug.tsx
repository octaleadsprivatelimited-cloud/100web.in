...
export const Route = createFileRoute("/industries/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
        meta: [
          { title: "Industry not found — 100 Web Technologies" },
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <Link
          to="/industries"
          className="mt-6 inline-flex items-center gap-2 rounded-full bg-brand-orange px-5 py-3 text-sm font-semibold text-brand-navy"
        >
...
function IndustryDetail() {
  const { industry, industries } = Route.useLoaderData() as {
    industry: ManagedIndustry;
    industries: ManagedIndustry[];
  };
  ...
  const proposalWhatsAppUrl = `https://wa.me/917780273879?text=${encodeURIComponent(`Hi 100 Web Technologies, I would like a free proposal for my ${industry.name} business.`)}`;
  const related = industries
    .filter((i) => i.category === industry.category && i.slug !== industry.slug)
    .slice(0, 6);
  const overviewImages =
    industry.slug === "fitness-center"
      ? [
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => setOverviewSlide((current) => (current + 1) % overviewImages.length),
      4200,
    );
  ...
  useEffect(() => {
    if (!overviewInView) return;
    const rail = overviewCarouselRef.current;
    const card = rail?.children[overviewSlide] as HTMLElement | undefined;
    if (!rail || !card) return;
  ...
  const Counter = ({ n, tone = "light" }: { n: number; tone?: "light" | "dark" }) => (
    <div className="mb-5 flex items-center gap-3 sm:mb-8">
      ...
      <span className={"h-px flex-1 " + (tone === "dark" ? "bg-white/15" : "bg-brand-navy/15")} />
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans">
      ...
      <section className="relative isolate overflow-hidden border-b border-slate-200 bg-[#b9caf9] bg-[url('/images/service-backgrounds/aurora-light.webp')] bg-cover bg-center py-8 sm:py-12">
        <div
          className={`mx-auto grid items-center gap-7 px-4 sm:px-6 lg:px-8 ${videoId ? "max-w-7xl lg:grid-cols-2" : "max-w-4xl"}`}
        >
          <div className="min-w-0 rounded-xl bg-[#f4f2fa]/95 p-6 shadow-[0_30px_60px_-20px_rgba(10,31,68,0.35)] ring-1 ring-white/60 sm:p-10">
            ...
            <h1 className="mt-3 text-4xl font-bold leading-[1.1] tracking-tight text-brand-navy sm:text-5xl">
              {industry.hero_title || (
                <>
                  Grow your <span className="text-brand-orange">{industry.name}</span> business
                  online.
                </>
              )}
            </h1>
            <p className="mt-5 max-w-xl text-lg text-brand-navy/75">
              {buildTagline(industry.name)}
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 rounded-full bg-brand-orange px-7 py-3.5 text-sm font-semibold text-brand-navy transition hover:brightness-110"
              >
                Get a free proposal <ArrowRight className="h-4 w-4" />
              </a>
              <a
                href="#results"
                className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white/70 px-7 py-3.5 text-sm font-semibold text-brand-navy transition hover:bg-white"
              >
                See the results
              </a>
              {industry.pdf_url && (
                <a
                  href={industry.pdf_url}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white/70 px-7 py-3.5 text-sm font-semibold text-brand-navy transition hover:bg-white"
                >
          ...
          <div
            className={
              videoId
                ? "relative aspect-video overflow-hidden rounded-xl border border-white/60 bg-black shadow-[0_24px_60px_rgba(10,31,68,0.22)]"
                : "hidden"
            }
          >
            {videoId ? (
            ...
            <div className="relative">
              <div className="grid gap-6 md:grid-cols-3 md:items-start md:gap-10">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-bold tracking-tight text-white sm:text-3xl md:text-4xl">
                    Built for {industry.name.toLowerCase()} owners
                  </h2>
                  <p className="mt-2 text-xs text-white/70 sm:mt-3 sm:text-sm">
                    Trusted by {content.audience} across India and abroad.
                  </p>
                </div>
                <p className="text-sm text-white/85 sm:text-base md:col-span-2 md:text-lg">
                  {buildOverview(industry.name)}
                </p>
              </div>
              ...
              <div className="mt-6 sm:mt-12">
                ...
                <div className="-mx-4 sm:hidden">
                  <div
                    ref={overviewCarouselRef}
                    className="flex snap-x snap-mandatory gap-3 overflow-x-auto scroll-smooth px-4 pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
                    ...
                    {overviewImages.map((image, i) => (
                      ...
                      >
                        {image.crop ? (
                          <div
                            role="img"
                            aria-label={`${industry.name} digital solution`}
                            className={`h-full w-full ${image.tone}`}
                            style={{
                              backgroundImage: `url(${image.src})`,
                              backgroundSize: image.imageSize ?? "400% 200%",
                          ...
                          <img
                            src={image.src}
                            alt={`${industry.name} digital solution ${i + 1}`}
                            loading="lazy"
                            draggable={false}
                            className={`h-full w-full select-none object-cover ${image.tone}`}
                          />
                        )}
                      </div>
                    ))}
                    ...
                    >
                      {image.crop ? (
                        <div
                          role="img"
                          aria-label={`${industry.name} digital solution`}
                          className={`h-full w-full transition duration-500 hover:scale-105 ${image.tone}`}
                          style={{
                            backgroundImage: `url(${image.src})`,
                            backgroundSize: image.imageSize ?? "400% 200%",
                            backgroundPosition: image.crop,
                        ...
                        <img
                          ...
                          alt={`${industry.name} digital solution ${i + 1}`}
                          loading="lazy"
                          className={`h-full w-full object-cover transition duration-500 hover:scale-105 ${image.tone}`}
                        />
                      )}
                    </div>
                  ))}
            ...
            <div className="relative">
              <div className="grid gap-6 md:grid-cols-2 md:items-start md:gap-10">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-bold tracking-tight text-white sm:text-3xl md:text-4xl">
                    The {industry.name} landscape today
                  </h2>
              ...
              */}
                  <p className="mt-2 text-sm text-white/85 sm:mt-3 sm:text-base">
                    We've worked with dozens of {industry.name.toLowerCase()} businesses — so we
                    skip the learning curve and start with what actually moves the needle for you.
                  </p>
                </div>
                <div>
                  ...
                  <ul className="mt-3 space-y-2 sm:mt-4 sm:space-y-3">
                    {content.challenges.map((c, index) => (
                      <li
                        key={c}
                        className={`flex items-start gap-2.5 rounded-lg border px-3 py-2.5 text-[13px] font-medium text-black shadow-sm sm:gap-3 sm:rounded-xl sm:px-4 sm:py-3 sm:text-sm ${["border-orange-300 bg-orange-200", "border-sky-300 bg-sky-200", "border-emerald-300 bg-emerald-200", "border-violet-300 bg-violet-200"][index % 4]}`}
                      >
          ...
          <div className="relative">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm text-white/80 sm:mt-3 sm:text-base">
                Everything you need to launch, capture leads and start converting in one transparent
                engagement.
              </p>
            </div>
              ...
              {pkg.map((item, index) => {
                ...
                return (
                  <button
                    key={item.title}
                    type="button"
                    onClick={() => setOpenPackage(isOpen ? null : index)}
                    aria-expanded={isOpen}
                    className="rounded-xl border border-white/15 bg-gradient-to-br from-black via-neutral-950 to-slate-900 p-3 text-left text-white transition hover:-translate-y-0.5 hover:border-brand-orange/70 sm:rounded-2xl sm:p-5"
                    style={{ boxShadow: "var(--shadow-card)" }}
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className="grid h-10 w-10 place-items-center rounded-lg bg-brand-orange text-black"
                        aria-hidden="true"
                      >
                        <Icon className="h-5 w-5" />
                      </span>
                      <h3 className="min-w-0 flex-1 text-sm font-bold leading-snug sm:text-base">
                        {item.title}
                      </h3>
                      <ChevronDown
                    ...
                    <p
                      className={`overflow-hidden text-xs text-white/75 transition-all sm:text-sm ${isOpen ? "mt-1.5 max-h-40 opacity-100" : "max-h-0 opacity-0"}`}
                    >
                      {item.desc}
                    </p>
                    <span className="mt-2 hidden text-[10px] font-bold uppercase tracking-wider text-white/50 sm:block">
          ...
          <div className="rounded-2xl border bg-card/60 p-5 sm:p-8">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-2 text-2xl font-bold tracking-tight sm:text-3xl md:text-4xl">
                Real outcomes for {industry.name} businesses
              </h2>
              <p className="mt-2 text-sm text-muted-foreground sm:mt-3 sm:text-base">
                Averages across our {industry.category.toLowerCase()} clients within the first 6
                months.
              </p>
            </div>
            <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-10 sm:gap-6 lg:grid-cols-4">
              {results.map((r, index) => (
                <div
                  key={r.label}
                  className="relative min-h-32 overflow-hidden rounded-xl border border-blue-400 bg-[#0b5ea8] p-4 text-white shadow-[0_12px_28px_rgba(9,62,136,0.28)] sm:min-h-44 sm:rounded-2xl sm:p-6"
                >
                  <div
                    aria-hidden="true"
                  ...
                  <div
                    aria-hidden="true"
                    className="absolute inset-0 bg-[linear-gradient(125deg,rgba(3,34,89,0.93),rgba(11,94,168,0.72),rgba(3,34,89,0.58))]"
                  />
                  <div className="relative flex h-full min-h-[6rem] flex-col justify-end sm:min-h-28">
                    <div className="text-2xl font-black text-white drop-shadow-sm sm:text-4xl">
                      {r.metric}
                    </div>
                    <p className="mt-1 text-xs font-medium leading-snug text-white/95 sm:mt-2 sm:text-sm">
                      {r.label}
                    </p>
                  </div>
                </div>
          ...
          <div className="relative">
            <div className="grid gap-6 md:grid-cols-2 md:items-start md:gap-10">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-bold tracking-tight text-white sm:text-3xl md:text-4xl">
                  Everything a modern {industry.name.toLowerCase()} needs
                </h2>
                <p className="mt-3 text-sm text-white/80 sm:mt-4 sm:text-base">
                  Purpose-built modules for {industry.category.toLowerCase()} — not a generic
                  template.
                </p>
                <a
                  href="#contact"
                  className="mt-4 inline-flex items-center gap-2 rounded-full bg-brand-navy px-5 py-2.5 text-sm font-semibold text-white transition hover:brightness-110 sm:mt-6 sm:px-6 sm:py-3"
                >
              ...
              <ul className="space-y-2 sm:space-y-3">
                {content.features.map((f) => (
                  <li
                    key={f}
                    className="flex items-start gap-2.5 rounded-lg border border-brand-orange bg-brand-orange px-3 py-3 text-[13px] font-semibold text-black sm:gap-3 sm:rounded-xl sm:px-4 sm:py-4 sm:text-sm"
                  >
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 md:py-14 lg:px-8">
        <Counter n={6} />
        <div
          className="relative overflow-hidden rounded-2xl border border-brand-navy/15 bg-brand-navy p-5 sm:p-8 text-white"
          style={{
        ...
        >
          <div
            aria-hidden
            className="absolute inset-0 bg-[linear-gradient(100deg,rgba(7,20,45,0.95),rgba(7,20,45,0.78),rgba(7,20,45,0.9))]"
          />
          <div className="relative">
            <div className="max-w-2xl">
              <p className="text-xs font-semibold uppercase tracking-wider text-brand-orange sm:text-sm">
              ...
              <h2 className="mt-2 text-2xl font-bold tracking-tight sm:text-3xl md:text-4xl">
                From first call to live site in 2–4 weeks
              </h2>
              <p className="mt-2 text-sm text-white/70 sm:mt-3 sm:text-base">
                A calm, weekly cadence — no surprises, no jargon, no chasing.
              </p>
            </div>
            <div className="mt-6 grid grid-cols-2 gap-2.5 sm:mt-10 sm:gap-6 lg:grid-cols-4">
              {deliveryProcess.map((p) => (
                <div
                  key={p.step}
                  className="rounded-xl border border-white/15 bg-white/10 p-3 backdrop-blur-md sm:rounded-2xl sm:p-6"
                >
                  ...
                  <p className="mt-1 text-[11px] leading-4 text-white/75 sm:mt-2 sm:text-sm">
                    {p.desc}
                  </p>
                </div>
              ))}
      ...
      {/* Package Includes: shown above as section 3 */}
          ...
          >
            ...
            <div className="relative">
              <div className="max-w-2xl">
                ...
                <p className="mt-2 text-sm text-white/80 sm:mt-3 sm:text-base">
                  Everything you need to launch, capture leads and start converting — bundled in a
                  single, transparent engagement.
                </p>
              </div>
              <div className="mt-6 grid grid-cols-1 gap-3 sm:mt-10 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
                {pkg.map((item, index) => {
          ...
          >
            ...
            <div className="relative">
              <div className="max-w-2xl">
                ...
                <p className="mt-2 text-sm text-white/80 sm:mt-3 sm:text-base">
                  A time-tested marketing framework — Attention, Interest, Desire, Action — applied
              ...
              <div className="mt-6 grid grid-cols-2 gap-2 sm:mt-10 sm:gap-4 lg:grid-cols-4">
                {strategy.map((s, i) => (
                  <div
                    key={i}
                    className="relative overflow-hidden rounded-xl border border-white/20 p-3 text-white sm:rounded-2xl sm:p-5"
                    style={{ boxShadow: "var(--shadow-card)" }}
                  >
                    <div
                      aria-hidden
                      ...
                      style={{
                        backgroundImage: `url(${aidaCardGradient})`,
                    ...
                    <div className="relative z-10">
                      ...
                      <h3 className="mt-2 text-xs font-bold leading-tight sm:mt-3 sm:text-lg">
                        {s.title}
                      </h3>
                      <p className="mt-1.5 hidden text-xs text-white/85 sm:block sm:text-sm">
                        {s.desc}
                      </p>
                    </div>
                  </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 md:py-14 lg:px-8">
        ...
        <div className="rounded-2xl border bg-card/40 p-5 sm:p-8">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm text-muted-foreground sm:mt-3 sm:text-base">
              A repeatable operating rhythm behind every growth campaign — no guesswork, no ad-hoc
              experiments.
            </p>
          </div>
        ...
        <div className="rounded-2xl border border-brand-navy/15 bg-brand-navy p-5 sm:p-8 text-white">
          <div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-end">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm text-white/70 sm:mt-3 sm:text-base">
                Indicative results from a 90-day engagement — actual numbers depend on budget,
                market and offer.
              </p>
            </div>
            <a
              href="#contact"
              className="inline-flex w-fit items-center gap-2 rounded-full bg-brand-orange px-6 py-3 text-sm font-semibold text-brand-navy transition hover:brightness-110"
            >
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-10 sm:gap-6 lg:grid-cols-5">
            {stats.map((s) => (
              <div
                key={s.label}
                className="rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur sm:rounded-2xl sm:p-6"
              >
          ...
          <div className="relative">
            <div className="grid gap-8 md:grid-cols-[1fr_1.2fr] md:items-start md:gap-12">
              <div>
                ...
                <p className="mt-3 text-sm text-white/80 sm:mt-4 sm:text-base">
                  We're not a generic agency running the same template on every client. We ship{" "}
                  {industry.name.toLowerCase()}-specific playbooks that consistently move leads,
                  revenue and repeat customers.
                </p>
                <a
                  href="#contact"
                  className="mt-5 inline-flex items-center gap-2 rounded-full bg-brand-navy px-6 py-3 text-sm font-semibold text-white transition hover:brightness-110"
                >
              ...
              <ul className="space-y-2 sm:space-y-3">
                {reasons.map((r) => (
                  <li
                    key={r}
                    className="flex items-start gap-2.5 rounded-lg border bg-card px-3 py-3 text-[13px] sm:gap-3 sm:rounded-xl sm:px-4 sm:py-4 sm:text-sm"
                  >
          ...
          <div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-center">
            <div className="min-w-0">
              ...
              <h3 className="mt-3 text-2xl font-bold sm:text-3xl">
                Ready to bring more customers to your {industry.name.toLowerCase()}?
              </h3>
              <p className="mt-2 text-white/70">
                Free 20-minute strategy call. No obligation, no pushy sales.
              </p>
            </div>
            <a
              href="#contact"
              className="inline-flex items-center justify-center gap-2 rounded-full bg-brand-orange px-7 py-3.5 text-sm font-semibold text-brand-navy transition hover:brightness-110"
            >
      ...
      <section id="contact" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-14 lg:px-8">
        <div
          className="overflow-hidden rounded-3xl border bg-card p-8 md:p-14"
          ...
          <div className="grid gap-10 md:grid-cols-2 md:items-start">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">
                Tell us about your {industry.name.toLowerCase()}
              </h2>
              <p className="mt-4 text-muted-foreground">
                Share a few details — a specialist who has worked with{" "}
                {industry.category.toLowerCase()} businesses will reply within one business day.
              </p>
              <div className="mt-6 space-y-3 text-sm">
                <div className="flex items-center gap-2">
                  <Star className="h-4 w-4 text-brand-orange" /> 500+ small-business clients
                  globally
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-4 w-4 text-brand-orange" /> Transparent pricing, no lock-ins
                </div>
                <div className="flex items-center gap-2">
                  <Star className="h-4 w-4 text-brand-orange" /> Ongoing support &amp; growth
                  partner
                </div>
              </div>
              <div className="mt-6 space-y-1 text-sm">
                <div>
                  ...
                  <a className="font-medium hover:text-brand-orange" href="mailto:hello@100web.in">
                    hello@100web.in
                  </a>
                </div>
                <div>
