  ...
  head: () => ({
    meta: [
      { title: "Industries — 100 Web Technologies" },
      {
        name: "description",
        content:
          "Solutions tailored to financial services, healthcare, retail, manufacturing, education, media, logistics and government.",
      },
      { property: "og:title", content: "Industries — 100 Web Technologies" },
      {
        property: "og:description",
        content: "Deep domain expertise across regulated and high-scale sectors.",
      },
...
function LazyIndustryPhoto({
  photo,
  alt,
}: {
  photo: { src: string; crop: string; imageSize: string };
  alt: string;
}) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label={alt}
      className="h-full w-full bg-[var(--brand-navy)] bg-cover transition duration-500 group-hover:scale-105"
      style={
        shouldLoad
          ? {
              backgroundImage: `url(${photo.src})`,
              backgroundSize: photo.imageSize,
              backgroundPosition: photo.crop,
            }
          : undefined
      }
    />
      ...
      <section className="bg-[var(--brand-navy)] text-white">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 md:py-14 lg:px-8">
          ...
          <h1 className="mt-3 max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl">
            Websites &amp; digital growth for 250+ industries.
          </h1>
          <p className="mt-5 max-w-2xl text-lg text-white/75">
            Pick your industry — every page has a tailored solution, real results, and a free
            proposal built for your business.
          </p>
        </div>
      </section>
                    ...
                    return (
                      ...
                      >
                        <div className="relative aspect-[16/8] overflow-hidden bg-[var(--brand-navy)]">
                          {photo ? (
                            <LazyIndustryPhoto photo={photo} alt={`${i.name} business`} />
                          ) : (
                            <img
                              src={`/api/industry-card/${i.slug}?card=industry-unique-v1`}
                              alt={`${i.name} business illustration`}
                        ...
                        <div className="flex items-center justify-between gap-3 px-4 py-3.5">
                          <span className="min-w-0 text-sm font-semibold text-[var(--brand-navy)] group-hover:text-[var(--brand-orange)]">
                            {i.name}
                          </span>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-between gap-4 rounded-2xl border bg-[var(--brand-navy)] p-6 text-white sm:p-8">
          <div>
            ...
            <p className="mt-1 text-sm text-white/70">
              We work with every kind of business. Tell us what you do.
            </p>
          </div>
          <Link
            to="/contact"
            className="inline-flex items-center gap-2 rounded-full bg-[var(--brand-orange)] px-6 py-3 text-sm font-semibold text-[var(--brand-navy)] hover:brightness-110"
          >
