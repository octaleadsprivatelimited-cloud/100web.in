...
function CustomerPortal() {
  ...
  return (
    <div className="min-h-screen bg-[#dfded8] p-3 sm:p-6">
      <header className="sticky top-3 z-30 mx-auto max-w-7xl rounded-2xl border border-white/70 bg-white/90 shadow-[0_12px_35px_rgba(24,31,38,0.08)] backdrop-blur">
      ...
      <main className="mx-auto mt-3 max-w-7xl rounded-[2rem] border border-white/70 bg-[#f8f8f6]/95 px-4 py-6 shadow-[0_20px_60px_rgba(24,31,38,0.10)] sm:px-6 lg:mt-5 lg:px-8 lg:py-8">
...
function Panel({ title, description, children }: { title: string; description: string; children: React.ReactNode }) { return <section className="rounded-[1.4rem] border border-slate-200/80 bg-white p-5 shadow-[0_10px_28px_rgba(24,31,38,0.05)] sm:p-6"><div className="mb-5"><h2 className="font-semibold text-brand-navy">{title}</h2><p className="mt-1 text-xs text-muted-foreground">{description}</p></div>{children}</section>; }
function Metric({ icon: Icon, label, value, hint }: any) { return <div className="rounded-[1.35rem] border border-slate-200/80 bg-white p-4 shadow-[0_10px_28px_rgba(24,31,38,0.05)]"><div className="flex items-center justify-between"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#f2f1ed] text-brand-navy"><Icon className="h-4 w-4" /></span><span className="text-2xl font-semibold text-brand-navy">{value}</span></div><p className="mt-4 text-xs font-semibold text-brand-navy">{label}</p><p className="mt-0.5 text-[11px] text-muted-foreground">{hint}</p></div>; }
